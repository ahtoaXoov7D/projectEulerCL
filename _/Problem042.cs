...
namespace ProjectEuler
{
    partial class ProjectEuler
    {
        ...
        static void P042()
        {
            ...
            var triangle = from i in Enumerable.Range(1, 30) select Functions.getTriangle(i);
            foreach (string word in File.ReadAllText(@"...\...\Resources\p042_words.txt").Split(',').Select(s => s.Replace("\"", "")).ToArray())
                if (triangle.Contains((from c in word select Convert.ToInt32(alphabetValues[c])).Sum())) ans++;
